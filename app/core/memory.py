...
class ConversationMemory:
    ...
    def _build_conversation_context(self, messages: List[Dict[str, Any]], exclude_last_user_message: bool = True) -> str:
        ...
        context_parts = []
        # Exclude the most recent user message if requested (since it's the current question)
        messages_to_use = messages[:-1] if exclude_last_user_message and messages and messages[-1].get("type") == "HumanMessage" else messages
        
        # Include all messages in the conversation (not just last 5)
        # This ensures the full conversation history is available for context
        for msg in messages_to_use:
            msg_type = msg.get("type") if isinstance(msg, dict) else msg.__class__.__name__
            msg_content = msg.get("content") if isinstance(msg, dict) else msg.content
            
            if msg_type == "HumanMessage":
                context_parts.append(f"User: {msg_content}")
            elif msg_type == "AIMessage":
                context_parts.append(f"Assistant: {msg_content}")
...
Please answer the current question considering the conversation context above. If the current question refers to something mentioned earlier (like "my previous question" or "what I asked before"), please use the conversation context to understand what they're referring to."""
    ...
    def _generate_response_node(self, state: ConversationState) -> ConversationState:
        ...
        try:
            ...
            ai_message = langchain_core.messages.AIMessage(content=answer)
